...
//    }
  ...
  public ResponseEntity<List<BookDTO>> getBooksByTitleContaining(@RequestParam @Size(min = 2) String title) {
    return ResponseEntity.ok(BookMapper.toBookDTOList(bookService.findAllBooksByTitleContaining(title)));
  }

  @GetMapping("/books/search/by-author")
  public ResponseEntity<List<BookDTO>> getBooksByAuthor(@RequestParam String author) {
    return ResponseEntity.ok(BookMapper.toBookDTOList(bookService.findAllBooksByAuthor(author)));
  }
  ...
  public ResponseEntity<Page<BookDTO>> getBooksPage(@RequestParam int page,
    ...
    Page<Book> pageOutput = bookService.findAllBooksPage(page, size, sort);
    return ResponseEntity.ok(BookMapper.pageToBookDTO(pageOutput));
  }
}
